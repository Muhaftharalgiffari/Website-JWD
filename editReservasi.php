...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['nomor_reservasi'])) {
    // Tangkap data yang diterima dari formulir
    $nomor_reservasi = $_POST['nomor_reservasi'];
    ...
    $sql = "UPDATE tabel_pemesanan SET nama_pemesan=?, no_telp=?, waktu_perjalanan=?, jumlah_peserta=?, pelayanan=?, harga_paket=?, total_tagihan=? WHERE nomor_reservasi=?";

    // Persiapkan statement SQL
    $stmt = mysqli_prepare($conn, $sql);

    // Bind parameter-parameter ke statement
    mysqli_stmt_bind_param($stmt, 'sssisdsi', $nama_pemesan, $no_telp, $waktu_perjalanan, $jumlah_peserta, $pelayanan, $harga_paket, $total_tagihan, $nomor_reservasi);

    // Eksekusi statement
    if (mysqli_stmt_execute($stmt)) {
        // Jika update berhasil, arahkan kembali ke halaman viewReservasi.php
        header("location: viewReservasi.php");
        exit();
    } else {
...
if (isset($_GET['nomor_reservasi'])) {
    $nomor_reservasi = $_GET['nomor_reservasi'];

    // Query untuk mengambil data reservasi berdasarkan nomor_reservasi
    $sql = "SELECT * FROM tabel_pemesanan WHERE nomor_reservasi=?";
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_bind_param($stmt, 'i', $nomor_reservasi);
        ...
        <body>
            <div class="container">
                ...
                <form action="editReservasi.php" method="post">
                    <input type="hidden" name="nomor_reservasi" value="<?php echo $row['nomor_reservasi']; ?>">
                    ...
                    <button type="submit" class="btn btn-primary">Simpan Perubahan</button> <a href="viewReservasi.php" class="btn btn-secondary">Kembali</a>
                </form>
            </div>
...
<?php
        ...
        echo "<p>Data reservasi tidak ditemukan.</p>";
    }
} else {
    echo "<p>Nomor reservasi tidak diberikan.</p>";
}
